    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col gap-2">
         <div className="flex items-center gap-2 text-primary">
            <Calendar size={18} />
            <span className="text-xs font-bold uppercase tracking-[0.2em]">Maio de 2026</span>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <motion.div variants={itemVariants} className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
               ...
               <div className="bg-white/5 border border-white/5 rounded-sm p-6">
                  <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                     <AlertCircle size={12} className="text-primary" /> Disponvel Hoje
                  </p>
                  <p className="text-2xl font-display font-bold text-white">{formatCurrency(summary.currentCash)}</p>
               </div>
            </div>
         </motion.div>
         ...
         <motion.div variants={itemVariants} className="space-y-6">
            <div className="bg-white/5 border border-white/5 rounded-sm p-6">
               <h4 className="text-sm font-bold text-white uppercase tracking-widest mb-6">Controle por Responsvel</h4>
               <div className="space-y-6">
                  <div className="flex items-center justify-between">
                     ...
                     <span className="text-sm font-bold text-white">{formatCurrency(useFinanceStore.getState().accounts.filter(a => a.owner === "Matheus").reduce((acc, a) => acc + a.balance, 0))}</span>
                  </div>
                  <div className="flex items-center justify-between">
                     <div className="flex items-center gap-3">
                     ...
                     <span className="text-sm font-bold text-white">{formatCurrency(useFinanceStore.getState().accounts.filter(a => a.owner === "Heloisa").reduce((acc, a) => acc + a.balance, 0))}</span>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-sm p-6">
               ...
               <p className="text-xs text-neutral-400 leading-relaxed">
                  "Vocês possuem {formatCurrency(summary.projectedEndBalance)} de sobra livre este mês. Que tal aplicar 50% desse valor na reserva de emergência hoje?"
               </p>
            </div>
         </motion.div>
      </div>
